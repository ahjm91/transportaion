  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-8" dir="rtl">
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        {/* Profile Details Edit (Modal or Inline) */}
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 space-y-4">
          <div className="flex items-center justify-between">
            <h4 className="text-sm font-black text-dark uppercase tracking-widest">تحديث بيانات المركبة</h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">رقم اللوحة</label>
              <input 
                type="text"
                placeholder="مثلاً: 1234 ABC"
                className="w-full bg-gray-50 border-none rounded-xl py-3 px-4 text-sm font-bold focus:ring-2 focus:ring-gold/20"
                value={driver.plateNumber || ''}
                onChange={async e => await updateDoc(doc(db, 'drivers', driver.id), { plateNumber: e.target.value })}
              />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">رابط صورة السيارة</label>
              <input 
                type="text"
                placeholder="https://..."
                className="w-full bg-gray-50 border-none rounded-xl py-3 px-4 text-sm font-bold focus:ring-2 focus:ring-gold/20"
          ...
          {driver.carImage && (
            <div className="mt-4">
               <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">معاينة الصورة:</p>
               <img src={driver.carImage} alt="Vehicle" className="w-full max-h-40 object-contain rounded-2xl border border-gray-100" />
            </div>
          )}
        </div>
        {/* Active Booking */}
